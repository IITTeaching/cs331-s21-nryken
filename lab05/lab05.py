...
class LinkedList:
    ...
    def cursor_move(self, offset):
        ...
        temp = self.head.next
        loc = offset
        while temp != self.cursor:
            loc += 1
            temp = temp.next
        while loc < 0:
            loc = loc + len(self)
        while loc > len(self) - 1:
            loc = loc - len(self)
        self.cursor_set(loc)
        ### END SOLUTION
    ...
    def cursor_delete(self):
        ...
        self.cursor = self.cursor.prior.next
        self.length -= 1
        ### END SOLUTION
    ...
    def insert(self, idx, value):
        ...
        ### BEGIN SOLUTION
        if self.length == 0:
            node = LinkedList.Node(value, self.head, self.head)
            self.head.prior = self.head.next = node
            self.length += 1
        elif idx == 0:
            node = LinkedList.Node(value, self.head, self.head.next)
            self.head.next = self.head.next.prior = node
            self.length += 1
        else:
    ...
    def index(self, value, i=0, j=None):
        ...
        indexer = self.head.next
        for _ in range(i):
            indexer = indexer.next
        for n in range(i, j):
            if indexer.val == value:
                return n
            indexer = indexer.next
        raise ValueError
    ...
    def __add__(self, other):
        ...
        ### BEGIN SOLUTION
        newList = LinkedList()
        newList.head.next = self.head.next
        newList.head.prior = self.head.prior
        newList.head.prior.next = other.head.next
        newList.head.prior = other.head.prior
        newList.length = len(other) + len(self)
        return newList
        ### END SOLUTION
